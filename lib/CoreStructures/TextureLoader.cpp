...
//		std::size_t found = textureFilePath.find_last_of("/\\");
		std::cout << "Failed the load the texture: " << textureFilePath << std::endl;
		return 0;
	}
...
GLuint TextureLoader::loadTexture(const std::string& textureFilePath, const TextureGenProperties& textureProperties)
{
	...
	if (newTexture) {
		...
		glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_MAX_ANISOTROPY, textureProperties.anisotropicLevel);
...
GLuint TextureLoader::loadCubeMapTexture(
	...
	glTexParameterf(GL_TEXTURE_CUBE_MAP, GL_TEXTURE_MAX_ANISOTROPY, anisotropicLevel);
	...
	for (int i = 0; i < 6; i++) {
        ...
        unsigned char* buffer = stbi_load(filename.c_str(), &t_width, &t_height, &t_nrChannels, 0);
		//Check if texture was loaded correctly
		if (!buffer)
		{
